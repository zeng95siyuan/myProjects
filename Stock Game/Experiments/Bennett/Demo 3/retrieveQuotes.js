...
function getQuote(symbol) {
    var url = `https://api.iextrading.com/1.0/stock/${symbol}/quote?filter=symbol, latestPrice`;
    request.get(url, (error, response, body) => {
        if (error) {	
            throw error;
        }
        var quote = JSON.parse(body);
        var sql = `UPDATE apitest3 SET price = ${quote.latestPrice} WHERE symbol = "${symbol}"`;
        con.query(sql, function (err) {
            if (err) throw err;
        });
    });
